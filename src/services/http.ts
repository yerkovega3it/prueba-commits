...
let tokenProvider: TokenProvider | null = () => localStorage.getItem("authToken")
...
http.interceptors.response.use(
  ...
  (error: AxiosError) => {
    ...
    const cfg = error.config as (InternalAxiosRequestConfig & { authRequired?: boolean }) | undefined
    const requiresAuth = cfg?.authRequired !== false; // default: true
    if ((status === 401 || status === 403) && requiresAuth && unauthorizedHandler) {
